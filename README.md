...
```php
之前也开发过一个simplePHP的框架，适用于PHP≥7.0。但是最近有一个商城的项目，服务器部署的PHP版本号是5.5，所以基于7.0的版本进行
修改出这个版本，这个版本适用于5.3~5.6，本人使用的版本是5.5，所以在index.php中限制了版本号。
```
...
```php
    ...
    2.模板标签
        ...
        程序初始化的时候调用register()方法，会把app & admin & wap三端的模板配置文件生成，开发时，只需要调用Helper.php:display & assign 
