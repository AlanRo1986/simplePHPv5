...
class TemplateProvider extends Provider {
    ...
    public function register()
    {
       ...
       */
        ...
        $this->cache_dir =  ROOT_PATH . conf('storage','cache'.ucfirst(TextUtils::lower($route->getApp())).'TplCache');
        $this->compile_dir =  ROOT_PATH . conf('storage','cache'.ucfirst(TextUtils::lower($route->getApp())).'TplCompiled');


        $this->assign('Controller',$route->getCtl());
     ...
     * @return mixed|string
     */
	protected function es_tmpl($out) {
     ...
     */
	protected function make_compiled($filename,$cache_id) {
     ...
     * @return string
     */
	protected function fetch_str($source) {
        return preg_replace_callback("/{([^\}\{\n]*)}/",function ($matches){
            return $this->select($matches[1]);
        },$source);
    }

    protected function getFileName($filename, $cached_id) {
        return basename ( $filename, strrchr ( $filename, '.' ) ) . '_' . $cached_id;
    }

    protected function getHashDirectory($filename, $cached_id) {
